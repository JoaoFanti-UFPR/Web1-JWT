...
const add1 = (request, response) => {
  ...
  try {
    ...
    response.json({ message: "Rota Add1 Acessada", user: decoded });
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

const add2 = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    response.json({ message: "Rota Add2 Acessada", user: decoded });
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

module.exports = { login, protectedContent, add1, add2 };
